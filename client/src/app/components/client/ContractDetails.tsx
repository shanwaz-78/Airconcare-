    ...
    return (
        <div className="bg-white shadow overflow-hidden sm:rounded-lg">
            ...
            <div className="px-4 py-4 sm:px-6 bg-gray-50">
                {contract.status === 'Quote Sent' && (
                    <AcceptQuoteButton contractId={contract.id} />
                )}
                {contract.status === 'Accepted by Client' && (
                    <PaymentButton contractId={contract.id} amount={contract.quoteAmount!} />
                )}
            </div>
        </div>
    )
